...
class Tableau:
    ...
    def result(self) -> np.ndarray:
        result = np.zeros(self.i + self.j, dtype=float)
    ...
    def __str__(self) -> str:
        headers = [
            ...
            "xb",
            "b",
            *[f"x{x+1}" for x in range(self.i + self.j)],
            "r",
        ]
...
def main():
    ...
    print(f"Optimum value: {np.linalg.matmul(ti.cj, ti.result)}")
